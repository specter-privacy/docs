...
<Frame>
  ![System architecture overview](/images/specter/system-architecture-overview.png)
</Frame>
...
<Zoomable label="sending-a-payment sequence diagram">
...
sequenceDiagram
    ...
    API->>Crypto: derive_stealth_address(spending_pub, shared_secret)
...
<Zoomable label="scanning-for-payments sequence diagram">
...
sequenceDiagram
    ...
    Recipient->>API: POST /stealth/scan {viewing_sk, spending_pub}
    ...
    loop For each announcement
        ...
        alt view_tag matches
            Scanner->>Crypto: derive_stealth_address(spending_pub, shared_secret)
            Crypto-->>Scanner: stealth_address (public only)
    ...
    API-->>Recipient: [{stealth_address, shared_secret, ...}]
    Note over Recipient: Derives the spend key locally:<br/>spending_sk + tweak(shared_secret)
```
