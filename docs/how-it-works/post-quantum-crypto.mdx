...
<Zoomable label="ML-KEM fit into SPECTER diagram">
...
1. **View tag derivation** - `SHAKE-256("SPECTER_VIEW_TAG_V1" || shared_secret)` produces a 1-byte filter
2. **Stealth tweak derivation** - `t = H_to_scalar("SPECTER_STEALTH_TWEAK_V2" || shared_secret || counter)` gives a secp256k1 scalar; the one-time address is `P = spending_pub + t·G`, spendable with `spending_sk + t`
...
<Frame>
  ![Classical vs ML-KEM](/images/specter/classical-vs-ml-kem.png)
</Frame>
...
<CardGroup cols={2}>
  <Card title="View tags and scanning" icon="dashboard" href="/how-it-works/view-tags-and-scanning">
