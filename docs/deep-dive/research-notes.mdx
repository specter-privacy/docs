...
ML-KEM shared secret
  → SHAKE-256("SPECTER_ETH_KEY_V1" || shared_secret || spending_pk)
    → secp256k1 seed (reduced to a valid scalar)
...
- `"SPECTER_STEALTH_PK_V1"` for stealth public key derivation
- `"SPECTER_STEALTH_SK_V1"` for the matching stealth secret key
- `"SPECTER_ETH_KEY_V1"` for the secp256k1 Ethereum spending key

The `_V1` suffix versions each label, leaving room for future derivations without ambiguity.
