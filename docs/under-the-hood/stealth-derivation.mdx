...
```
material = SHAKE-256("SPECTER_STEALTH_PK_V1" || shared_secret)   bound to spending_pk
```

The `"SPECTER_STEALTH_PK_V1"` prefix is the domain separator. It guarantees this output is independent of the [view tag](/under-the-hood/scanning-and-spending), which is derived from the same shared secret under a different prefix. The spending public key is mixed in so the resulting address is tied to one recipient. The matching secret path uses `"SPECTER_STEALTH_SK_V1"`, and the Ethereum spending key is derived under `"SPECTER_ETH_KEY_V1"` over `shared_secret || spending_pk`. The `_V1` suffix versions each label so future derivations stay unambiguous.
...
flowchart TD
    ...
    ETH --> ADDR["Eth + Sui stealth address (sender needs this)"]
