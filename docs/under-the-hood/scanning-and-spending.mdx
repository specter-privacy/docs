...
<Zoomable label="Scanning announcements">
```mermaid
flowchart LR
...
<Warning>
The recovered private key spends real funds. Keep it inside the signing path. Do not log it, store it in plaintext, or send it to any service. The SDK scans locally: decapsulation and key recovery run on the device, and no secret key is ever transmitted. See the [SDK security model](/sdk/security).
</Warning>
...
- On **Sui**, the same secp256k1 key signs under Sui's secp256k1 scheme, usable with the Sui SDK.
...
Receiving is post-quantum: the shared secret behind every announcement is protected by [ML-KEM-768](/under-the-hood/shared-secret). The spend signature is classical: secp256k1 on both Ethereum and Sui. A future quantum computer could attack those signatures, but only after the key is used on-chain, by which point the funds have moved. The protocol currently prioritizes the receiving path because announcement data is permanent and public, while a spend is a one-time event. The full reasoning and the migration path are in [security boundaries](/how-it-works/security-boundaries).
