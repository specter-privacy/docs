...
stealth_key   = SHAKE-256("SPECTER_STEALTH_PK_V1" || shared_secret)  combined with spending_pk
```

The domain strings are the separator, and the `_V1` suffix versions them so the protocol can introduce new derivations later without ambiguity. The same shared secret feeds both, but the prefixes guarantee the view tag tells you nothing about the stealth key. The exact constructions live in the SPECTER Rust core; this section describes what each one produces and why.
