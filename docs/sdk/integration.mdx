...
const results = scanAnnouncements(
  ...
  recipient.viewing,
  recipient.spending.publicKey,
  recipient.spending.secretKey, // recover the spendable stealthKeys on matches
);
...
Earlier versions exposed `generateKeysRemote()` and `scanRemote()`, which could send secret key material to the server. **They were removed in v1.0.0.** The HTTP client is now public-data only (`createStealthPaymentRemote`, `publishAnnouncement`). Generate keys with `generateSpecterKeys()` and scan with `scanAnnouncement()` / `scanAnnouncements()` — both run entirely on the device and never transmit a secret key. See the [security model](/sdk/security).
