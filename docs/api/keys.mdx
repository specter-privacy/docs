...
<CardGroup cols={2}>
  <Card title="Single-call key generation" icon="lock">
    `POST /api/v1/keys/generate` returns spending and viewing keypairs plus a meta-address.
  </Card>
  <Card title="Use output in next steps" icon="route-2" href="/api/stealth">
...
Hex-encoded spending public key (33-byte compressed secp256k1).
</ParamField>
...
Hex-encoded SPECTER meta-address combining spending + viewing public keys (1218 bytes).
</ParamField>
<ParamField body="protocol_version" type="integer" required>
SPECTER protocol version encoded in the meta-address (currently `2`).
</ParamField>
...
<AccordionGroup>
  <Accordion title="How `view_tag` is computed here">
    It is not computed here. Every announcement gets its own per-payment `view_tag` from the ML-KEM shared secret at stealth creation time.
  </Accordion>
