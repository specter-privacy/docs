...
<Steps>
  ...
  <Step title="Publish announcement" icon="bell">
    `POST /api/v1/registry/announcements` with the returned `payment_id` after the source-chain payment.
  </Step>
...
- **Security headers** (`X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy`, legacy XSS protection)
