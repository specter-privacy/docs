...
<AccordionGroup>
  ...
  <Accordion title="Metadata encryption">
    ...
    Encryption uses AES-256-GCM and adds a 16-byte authentication tag over bytes 1-76, leaving the `view_tag` in the clear: 1 + 76 + 16 = 93 bytes. The `@specterpq/sdk` produces the same block client-side. See [announcement metadata](/sdk/api-reference#announcement-metadata).
  </Accordion>
</AccordionGroup>
