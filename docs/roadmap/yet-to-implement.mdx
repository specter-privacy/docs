...
<CardGroup cols={3}>
  ...
  <Card title="Four chains, testnet-first" icon="network">
    Payments on Ethereum, Arbitrum, Monad, and Sui, with Monad serving as the announcement registry chain.
  </Card>
</CardGroup>
...
<Steps>
  ...
  <Step title="L2 chain expansion" icon="network">
    **Priority: High**
    ...
    Bring stealth addresses to Base and Optimism. Ethereum, Arbitrum, and Monad are already live. The same meta-address already derives valid addresses for any EVM chain, so the remaining work is registry support, frontend chain-switching, and gas optimization for each new network.
  </Step>
