...
<Tabs>
  <Tab title="API-first">
    ...
    **Best for:** Wallets, dApps, payment platforms that want to add stealth receiving.
    ...
    // Send funds to payment.stealth_address, then publish announcement with payment.payment_id
    ```
  </Tab>
...
sequenceDiagram
    ...
    API-->>App: {payment_id, stealth_address, ciphertext, view_tag}
    App->>Chain: Send ETH/tokens to stealth_address
    App->>API: POST /registry/announcements {payment_id, tx_hash}

    Note over App: Periodic (recipient side)
    App->>API: POST /stealth/scan {viewing_sk, spending_pub}
    API-->>App: [{stealth_address, shared_secret, ...}]
    App->>App: Derive spend key locally (spending_sk + tweak)
```
...
const resolved = await fetch("https://backend.specterpq.com/api/v1/ens/resolve/alice.eth")
  .then(r => r.json());
