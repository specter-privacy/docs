...
<AccordionGroup>
  <Accordion title="What is ML-KEM-768?">
    ML-KEM-768 (formerly Kyber) is a **post-quantum key encapsulation mechanism**. It lets two parties establish a shared secret even if an attacker has a quantum computer. SPECTER uses it to create the one-time stealth addresses.
