...
<Steps>
  <Step title="Setup — Generate Your Identity" icon="key">
    Open **[specterpq.com/setup](https://specterpq.com/setup)** and generate your recipient identity. The app creates your **meta-address** — a public profile containing your spending and viewing public keys.
  ...
  <Step title="Send — Create a Stealth Payment" icon="paper-plane">
    Open **[specterpq.com/send](https://specterpq.com/send)**. Enter the recipient's ENS/SuiNS name (or paste their meta-address directly). The app creates a **one-time stealth address** — a fresh destination that only the recipient can discover.
  ...
  <Step title="Scan — Discover Your Payments" icon="magnifying-glass">
    Open **[specterpq.com/scan](https://specterpq.com/scan)**. The app scans all announcements using your private viewing key and reveals only the payments meant for you. View tags filter out ~99.6% of irrelevant entries before decapsulation, keeping scanning very fast.
...
![SPECTER product flow map](/images/specter/protocol-flow-visual.png)
...
| --- | --- | --- |
| Setup keys | [/setup](https://specterpq.com/setup) | `POST /api/v1/keys/generate` |
| Send payment | [/send](https://specterpq.com/send) | `POST /api/v1/stealth/create` |
| Publish | — | `POST /api/v1/registry/announcements` |
| Scan | [/scan](https://specterpq.com/scan) | `POST /api/v1/stealth/scan` |
