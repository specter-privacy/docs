...
<Warning>
Current Yellow endpoints are integration scaffolding. Several responses are placeholders and this backend does not submit L1 settlement transactions.
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="No backend L1 close submission">
    Close handler does not submit an L1 transaction. The frontend sends a close to Yellow processing, and settlement is expected from Yellow infrastructure outside this backend.
  </Accordion>
</AccordionGroup>
...
- Endpoint implementations: `specter/specter-api/src/handlers.rs`
- Yellow defaults and env overrides: `specter/specter-api/src/state.rs`
